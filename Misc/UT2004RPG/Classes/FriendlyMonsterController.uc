...
var FriendlyMonsterSkinner Skinner;
...
function SetMaster(Controller NewMaster)
{
	...
	if (Master.PlayerReplicationInfo != None && Master.PlayerReplicationInfo.Team != None)
	{
		...
		if (PlayerReplicationInfo.Team.TeamIndex < 2)
		{
			Skinner = spawn(class'FriendlyMonsterSkinner', Pawn);
			Skinner.Team = PlayerReplicationInfo.Team.TeamIndex;
			if (Level.NetMode != NM_DedicatedServer)
				Skinner.CreateMonsterSkins();
		}
	}
}
...
function bool FindNewEnemy()
{
	...
	for (C = Level.ControllerList; C != None; C = C.NextController)
		if (C != Master && C != self && C.Pawn != None && (FriendlyMonsterController(C) == None || FriendlyMonsterController(C).Master != Master) && !C.SameTeamAs(Master) && VSize(C.Pawn.Location - Pawn.Location) < BestDist && CanSee(C.Pawn))
		{
			BestEnemy = C.Pawn;
...
function ExecuteWhatToDoNext()
{
	...
	if ( Enemy != None )
		ChooseAttackMode();
	else if (Master != None && Master.Pawn != None)
		FollowMaster();
	else
	{
