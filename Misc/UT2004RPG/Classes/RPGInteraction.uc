...
{
	local float XL, YL, XLSmall, YLSmall;
	...
	Canvas.FontScaleX = Canvas.ClipX / 1024;
	Canvas.FontScaleY = Canvas.ClipY / 768;
	Canvas.TextSize("Level:", XL, YL);

	XL = 129 * Canvas.FontScaleX;
	Canvas.Style = 5;
	Canvas.DrawColor = EXPBarColor;
	Canvas.SetPos(Canvas.ClipX - XL - 1, Canvas.ClipY * 0.75 - YL * 3.75);
	Canvas.DrawTile(Material'InterfaceContent.Hud.SkinA', XL * StatsInv.Data.Experience / StatsInv.Data.NeededExp, 12.0 * Canvas.FontScaleY, 836, 454, -386 * StatsInv.Data.Experience / StatsInv.Data.NeededExp, 36);
	...
	Canvas.SetPos(Canvas.ClipX - XL - 1, Canvas.ClipY * 0.75 - YL * 3.75);
	Canvas.DrawTile(Material'InterfaceContent.Hud.SkinA', XL, 12.0 * Canvas.FontScaleY, 836, 454, -386, 36);
	Canvas.DrawColor = WhiteColor;
	Canvas.SetPos(Canvas.ClipX - XL - 1, Canvas.ClipY * 0.75 - YL * 3.75);
	Canvas.DrawTile(Material'InterfaceContent.Hud.SkinA', XL, 12.7 * Canvas.FontScaleY, 836, 415, -386, 38);

	Canvas.Style = 2;
	Canvas.DrawColor = WhiteColor;
	Canvas.SetPos(Canvas.ClipX - XL - 1, Canvas.ClipY * 0.75 - YL * 5.0);
	Canvas.DrawText("Level: "$StatsInv.Data.Level);
	Canvas.FontScaleX *= 0.5;
	Canvas.FontScaleY *= 0.5;
	Canvas.TextSize(StatsInv.Data.Experience$"/"$StatsInv.Data.NeededExp, XLSmall, YLSmall);
	Canvas.SetPos(Canvas.ClipX - XL * 0.5 - XLSmall * 0.5, Canvas.ClipY * 0.75 - YL * 3.75 + 12.7 * Canvas.FontScaleY - YLSmall * 0.5);
	...
	if (bDefaultBindings)
	{
		Canvas.TextSize("Press L for stats/levelup menu", XL, YL);
		Canvas.SetPos(Canvas.ClipX - XL - 1, Canvas.ClipY * 0.75 - YL * 1.25);
		Canvas.DrawText("Press L for stats/levelup menu");
	...
	if (RPGArtifact(ViewportOwner.Actor.Pawn.SelectedItem) != None)
	{
		...
		if (bDefaultArtifactBindings)
		{
			...
			Canvas.DrawText("U to use, brackets to switch");
		}
	}
