...
namespace TheGame.Parsers.Commands
{
	...
	public class Connect : Command
	{
		...
		/// </summary>
		/// <param name="source">The extra parameters to the connect command.</param>
		public Connect(StringTokenizer source)
		{
		...
		{
			CommandFactory.RegisterCommand(new Pair<string,GenerateCommand>("connect",CreateConnectCommand));
			return;
		}
		...
		/// </summary>
		/// <param name="source">The extra parameters to the connect command.</param>
		/// <returns>Returns a connect command.</returns>
		protected static Command CreateConnectCommand(StringTokenizer source)
		...
		public bool Execute(Window win, GameState state)
		{
			if(valid)
			{
				...
				nio.Write("soineslksasfahslfhldsalfksjd",Join);

				if(!nio.WaitFor(str => {if(str == "asldfkjaslkefjslkjsestslo") return true; return false;},1000))
					return false;
				...
				// Recieve random seed and create RNG in state
				nio.WaitFor(str => {if(str.Substring(0,37) == "asdkflskjdfaosiejflxkdf;oaij9alksjdfi") return true; return false;});
				state.rand = new Random(int.Parse(nio.NextPacket().Data.Substring(37)));
		...
		public void Man(Window win, Rectangle Output)
		{
			...
			win.Write("connect - Connects to a host computer to start a game (you must also have a unit with leadership).",Output);
