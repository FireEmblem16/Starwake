...
{
	/// <summary>
	/// Represents a flying unit's natural protection.
	/// Note that this is only obtainable by being a flying unit so we do not initialize the class.
	/// </summary>
	public class FlyingArmor : DamageModifier
	{
		/// <summary>
		/// Creates a new flying armor ability.
		/// </summary>
		public FlyingArmor()
		{
			Level = 1;
		...
		{
			if((type & (ATTACK_TYPE.FLYING | ATTACK_TYPE.RANGED)) == ATTACK_TYPE.TYPELESS)
				damage *= 0.5f;
		...
		{
			get
			{return "Aerial Evasion\n--------------\n\tA flying unit's armor.\n\tReduces all damage from non-ranged and non-flying attacks by half.";}
		}

		/// <summary>
		/// The source name of the ability.
		/// </summary>
		protected const string name = "Aerial Evasion";
