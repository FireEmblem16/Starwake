...
#region Constructors
        ...
        public HeartsGameState(bool north_human = true, bool east_human = false, bool south_human = false, bool west_human = false, bool hundred_reset = false, AIBehavior AI = null)
        {
...
#region HelperFunctions
		...
		{
			if(move.Player != ActivePlayer || !GetPlayer(move.Player).CardsInHand.Cards.Contains(move.Play))
				return false;
		...
		protected void CopyEventsTo(GameState new_state)
		{
			...
			if(Passing != null)
				foreach(Delegate d in Passing.GetInvocationList())
					(new_state as HeartsGameState).Passing += d as PassingEvent;
