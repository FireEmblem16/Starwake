...
namespace Networking
{
    ...
    /// by Can Liang
    /// </author>
    public class SymetricNetworkStream : NetworkStream
    {
        ...
        public SymetricNetworkStream(bool isClient)
        {
            if (isClient)
            {
        ...
        {
            Listener = new Socket(AddressingType, DataType, Protocol);
		...
		/// <summary>
		/// Closes the network and permenantly terminates all functionality.
		/// </summary>
		public void Close()
		{
			Disconnect();
        ...
        public string PeekString()
        {
            ...
            {
				try
				{ // You have threading issues (at the very least make dataQueue volatile)
					string temp = Encoding.ASCII.GetString(dataQueue[0], 0, dataQueue[0].Length);
					return temp;
				}
				catch
