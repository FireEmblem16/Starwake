...
/**
 * Moves all selected items to the selected location.
 */
public class MoveTool extends Tool
{
	 ...
	 */
	public MoveTool(ArrayList<Location> selected_locations, Pointer<Location> center, JComboBox type, JComboBox Class, JSpinner size, String image)
	{
	...
	/**
	 * Moves all of the selected items to the selected location. Using this tool does not
	 * unselect items but rather changes the selection.
	 */
			...
			if(IsAllLayers())
			{
				...
				while(t != null)
				{
					for(int i = 0;i < items.size();i++)
					{
						...
						Location old_loc = item.GetLocation();
						...
						if(!new_locs.contains(old_loc))
							map.RemoveItem(t,old_loc);
