...
#pragma region Test Dependancies
	...
	out = fopen("known_mix.exe","rb");

	if(out == (PFILE)NULL || out == (PFILE)BAD_PTR || out == (PFILE)UNDEF_PTR)
	{
		fputs("This program requires an additional executable by the name of known_mix.exe to be in the running directory.\n",stderr);
		return EXIT_FAILURE;
	}
...
#pragma region Finding Mix
		...
		fputs("Searching for mix.exe.\n\0",stdout);
		err = find_file("mix.exe",FALSE,FALSE);

		if(err == 0xFFFFFFFF)
		{
			fputs("Unable to find mix.exe.\nNow searching for mix.\n",stdout);
			err = find_file("mix",FALSE,FALSE);
		...
		/* We're using err as a value for if we've found gensine: 1 is mix, 2 is mix.exe */
		switch(err)
		{
		case 1:
			fputs("Copying mix.exe to the running directory.\n",stdout);

			cmd[0] = '\0';
			strcat(cmd,"cp \"");
			strcat(cmd,get_directory());
			strcat(cmd,"/mix.exe\" mix.exe");

			if(system(cmd))
			{
				fputs("Unable to copy mix.exe in ",stderr);
			...
			break;
		case 2:
			fputs("Copying mix to the running directory.\n",stdout);

			cmd[0] = '\0';
			strcat(cmd,"cp \"");
			strcat(cmd,get_directory());
			strcat(cmd,"/mix\" mix.exe");

			if(system(cmd))
			{
				fputs("Unable to copy mix in ",stderr);
				fputs(get_directory(),stderr);
				fputs(" to running directory.\n",stderr);
		...
		case 0:
		default:
			fputs("Unable to find the gensine executable for this student.\mix score: 0/170 points.\n",out);
			b = FALSE;
...
#pragma endregion
		...
		/* If b true then we found the mix program */
		/* Scoring - Out of 170 */
		/* 85 points each for two successful runs */
		if(b)
		{
...
#pragma region Mix Test 1
			fputs("Attempting first mix test.\n",stdout);

			script = fopen("temporary_script.sh","wb");
			fputs("#!/bin/bash\n",script);
			fputs("./mix.exe known1.su known2.su > test.su",script);
			fclose(script);

			system("chmod u+x temporary_script.sh");
			system("bash ./run_temporary_script.sh");
			__sleep(CLOCKS_PER_SEC * 10); /* Ten seconds is a very generous time for mix to finish */
...
#pragma endregion
			...
			if(system("rm test.su test.wav analysis.out"))
			{
				fputs("Unable to remove mix test files.",stderr);
				return EXIT_FAILURE;
			}

			if(system("rm mix.exe temporary_script.sh pid"))
			{
				fputs("Unable to remove temporary files from mix test.\n",stderr);
				return EXIT_FAILURE;
			}
