...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
#ifndef _PILL_SEEKER_CPP
#define _PILL_SEEKER_CPP

#include "../Headers/PillSeeker.h"

PillSeeker::PillSeeker() : targets(), last_location(0,0)
{
...
MoveBehaviour* PillSeeker::CreateNew()
{
	return new PillSeeker();
}

// Looks into the spaces in info and adds any pills found.
void PillSeeker::AddNewTargets(MoveInfo* info)
{
	// Look for energy pills to add
	for(size_t i = 0;i < info->Visables();i++)
		if(info->GetVisable(i).type == "energy-pill")
		{
			// Get the ith data
...
void PillSeeker::RemoveTarget(Target rm)
{
	// Look for rm in info
...
bool PillSeeker::HasTarget(Target p)
{
	for(uint32_t i = 0;i < targets.size();i++)
...
Target PillSeeker::GetTarget(Target here)
{
...
void PillSeeker::SearchAndDestroy(MoveInfo* info, Target here, Target dim)
{
...
Target PillSeeker::GetDirection(Target here, Direction dir)
{
	switch(face)
	{
	case NORTH:
...
bool PillSeeker::OnGrid(Target p, Target dim)
{
...
Target PillSeeker::DetermineMoveDirection(MoveInfo* info, Target here, Target there, Target last, Target dim)
{
	// We will return this
	Target ret = here;
...
void PillSeeker::Rotate(Direction d)
{
	if(d != LEFT && d != RIGHT)
		return;
...
string PillSeeker::Move(MoveInfo* info, uint32_t x, uint32_t y, uint32_t last_x, uint32_t last_y, uint32_t width, uint32_t height)
{
...
long PillSeeker::rand(long upperbound, long min)
{
